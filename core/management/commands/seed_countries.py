...
# List of all countries (ISO codes + names)
COUNTRIES_DATA = [
...
class Command(BaseCommand):
    help = '🌍 Seeds all world countries into the database'

    def handle(self, *args, **options):
        self.stdout.write(self.style.WARNING('🌍 Seeding all countries...'))
        ...
        for code, name in COUNTRIES_DATA:
            country, created = Country.objects.get_or_create(
                name=name,
                defaults={'language': 'English', 'nationality': name}
            )
            if created:
                created_count += 1
            else:
        ...
        self.stdout.write(self.style.SUCCESS(
            f'✅ Countries seeded: {created_count} created, {updated_count} already existed'
        ))
